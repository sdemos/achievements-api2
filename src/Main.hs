...
site =
    ...
    where
        ...
        post = route [ ("users/:userName", createUser),
                       ("apps/:appName/:userName/:achievementid/:achievementProgress", updateAchievement)
                     ]
...
getUserAchievementProgress appName userName achievementid = do
  progress <- map fromSql (getQuery "SELECT t1.progress, t2.progress_max FROM (achievement_progress AS t1 INNER JOIN achievements AS t2 ON t1.achievement_id=t2.id INNER JOIN apps on apps.id = t2.app_id) JOIN users AS t3 ON t1.user_id=t3.id WHERE t3.username=(?) AND apps.name like (?)" [toSql userName, toSql appName] !! 0)
...
updateAchievement = do
  ...
  achievementProgress <- safeGetParam "achievementProgress"
...
updateProg appName userName achievementid achievementProgress currProg
    ...
    | currProg !! 0 < currProg !! 1 = "Old Progress - " ++ show $ currProg !! 0 ++ "\nNew Progress - " ++ show $ currProg !! 0 + achievementProgress
